%% blah blah blah
function [gamma_value,numerator,denuminator,denuminator_sudo,W_uc,coeff_vector ]= gamma_func(data,data_c,data_u,data_u_neighs,data_u_cats,data_u_neighs_cats,model,w,c,l,t,eta_u,a_u,ngbs_u,method)%% denuminator_modified and W_uc is applcable when v=0
...
%                if(strcmp(method,'stil')==0)
                 ...
                 elseif(spatial_etk_period_consideration==1)
                     W_uc_matrix_spec_element=W_uc_matrix_spec_element+a_u(node_ind)*exp(-model.sigma_location_exponential*abs(t-model.w-ti));
                 end
       end
      ...
      elseif(spatial_etk_period_consideration==1)
            W_uc_matrix_sum=W_uc_matrix_sum+a_u(node_ind)*exp(-model.sigma_location_exponential*(t-model.w-ti));
       
            coeff_vector(node_ind)=coeff_vector(node_ind)+exp(-model.sigma_location_exponential*(t-model.w-ti));
      end
end
